...
class Application(ttk.Frame):   #メインフレーム Frameクラスを継承  
    InputTableList = [] #InputTableListを初期化
    class InputTable:
        InputTableNum = 0
        ...
        def __init__(self,ParentFrame,frame_TimeTable_Table):                  # コンストラクタ
            ...
            button_Time = Button(ParentFrame, text="Enter",command=self.DrawRect(frame_TimeTable_Table, 
    ...
    def ApendInputTable(self, ParentFrame, frame_TimeTable_Table,frame_button_TimeInput):
        def inner():
            frame=Frame(ParentFrame, relief=FLAT,bg='gray',width=1000,height=100)
            frame.grid(row=self.InputTable.InputTableNum + 1,column=0)
            self.InputTableList.append(self.InputTable(frame, frame_TimeTable_Table))
